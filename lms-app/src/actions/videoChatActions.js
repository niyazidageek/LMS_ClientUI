...
export const setUserAction =
  (user, participants, roomId) => async (dispatch) => {
...
export const addParticipantAction =
  (user, currentUser, mainStream, roomId) => (dispatch) => {
    const newUserId = Object.keys(user)[0];
    const currentUserId = Object.keys(currentUser)[0];

    if (mainStream && currentUserId !== newUserId) {
      user = addConnection(user, currentUser, mainStream, roomId);
    }

    if (currentUserId === newUserId) user[newUserId].currentUser = true;
...
export const updateUserAction =
  (currentUser, user, roomId) => async (dispatch) => {
